...
export default function Grid({ items }: { items: GridItem[] }) {
  return (
    <div className="grid w-full py-10 grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
      <AnimatePresence mode="wait">
        {items.map((item, i) => (
          <motion.div
            key={`${item.title}-${i}`}
            whileHover={{ scale: 1.02 }}
            className="text-white relative bg-slate-800 hover:bg-slate-700 rounded-xl p-5"
          >
            <img
              src={item.photo}
              className="rounded h-1/2 w-full "
              alt={item.title}
            ...
            <div className="p-2">
              ...
              <p className="text-white-100 text-base mb-4 text-pretty">
                {item.description}
              </p>
